# ==========================================
# Motor Speed Control via Hand Gestures
# Using Python (OpenCV + MediaPipe) and Arduino Serial Communication
# ==========================================
...
# Function to calculate the angle at joint `b` formed by points `a-b-c`.
def calculate_angle(a, b, c):
    """Calculate the angle (in degrees) between three points (a, b, c).

    The angle returned is the smaller angle between BA and BC.
    """
    ba = [a[0] - b[0], a[1] - b[1]]
    bc = [c[0] - b[0], c[1] - b[1]]
    ...
    # Clamp cosine to the valid range [-1, 1] to avoid domain errors.
...
# Helper functions for finger/gesture analysis
def is_finger_curled(mcp, pip, dip, tip):
    """Return True if the finger (defined by its joint coordinates) is curled.

    The check uses the angles at the PIP and DIP joints and treats angles
    smaller than ~160 degrees as curled.
    """
    angle_pip = calculate_angle(mcp, pip, dip)
    angle_dip = calculate_angle(pip, dip, tip)
    return angle_pip < 160 or angle_dip < 160

# Gesture detection: thumb-up
def is_thumb_up(handLms, img_shape):
    """Return True for a thumbs-up gesture (thumb extended, other fingers curled)."""
    h, w, c = img_shape
...
# Gesture detection: thumb-down
def is_thumb_down(handLms, img_shape):
    """Return True for a thumbs-down gesture (thumb down, other fingers curled)."""
    h, w, c = img_shape
...
# Gesture detection: raised fist
def is_raised_fist(handLms, img_shape):
    """Return True for a raised fist (all fingers curled and palm facing camera)."""
    h, w, c = img_shape
...
# Gesture detection: raised hand (open palm)
def is_raised_hand(handLms, img_shape):
    """Return True for an open raised hand (palm/wrist oriented as 'raise')."""
    h, w, c = img_shape
...
# Gesture detection: "love you" (index and pinky extended)
def is_love_you(handLms, img_shape):
    """Return True for the 'love you' sign (index and pinky extended, middle and ring curled)."""
    h, w, c = img_shape
...
def is_okay_hand(handLms, img_shape):
    """Return True for an OK sign (thumb and index form a ring, other fingers relaxed/curled)."""
    h, w, c = img_shape
...
# Optional speed helper methods (currently unused)
# def increase_speed():
#     """Increase the global `speed` value by a step, capped at 255."""
#     global speed
#     if speed < 255:
#         speed += 50
#         speed = min(speed, 255)
...
# def decrease_speed():
#     """Decrease the global `speed` value by a step, floored at 0."""
#     global speed
#     if speed > 0:
#         speed -= 50
#         speed = max(speed, 0)
...
# Map detected hand landmarks to gesture names and display messages
